...
__petercordpanda {petercordpanda_version},  by @diemmmmmmmmmm
"""
...
_start = [
    [
        Button.inline("🔷 ʙᴀʜᴀsᴀ", data="lang"),
...
@asst_cmd("start ?(.*)")
async def petercordpanda(event):
    if event.is_group:
        ...
        if str(event.sender_id) not in owner_and_sudos():
            ...
            if not udB.get("STARTMSG"):
                ...
                await event.reply(
                    f"🐼 hai [{get_display_name(u)}](tg://user?id={u.id}), ini adalah 🐼 Panda asisstant bot milik [{petercordpanda_bot.me.first_name}](tg://user?id={petercordpanda_bot.uid})!\n\n{ok}",
                    buttons=[Button.inline("Info.", data="ownerinfo")],
                )
            else:
                me = f"[{petercordpanda_bot.me.first_name}](tg://user?id={petercordpanda_bot.uid})"
...
async def botstat(event):
    ...
    msg = """PandaUserbot asisstant - statistik
