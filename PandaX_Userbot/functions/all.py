...
telegraph = Telegraph()
telegraph.create_account(short_name="PetercordPanda Cmds List")
...
async def updater():
    ...
    try:
        ...
        await petercordpanda_bot.asst.send_message(
...
async def create_token_file(token_file, event):
    ...
    async with petercordpanda_bot.asst.conversation(ultroid_bot.uid) as conv:
        await event.edit(
...
async def restart(ult):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        ...
        execl(executable, executable, "-m", "pyPetercordPanda")
            ...
            if "(" not in downloaded_file_name:
                ...
                try:
                    ...
                    if plug in HELP:
                        ...
                        await ok.edit(
                            f"✓ `PetercordPanda - Installed`: `{plug}` ✓\n\n{output}"
                        )
...
async def allcmds(event):
    ...
    t = telegraph.create_page(title="PetercordPanda All Cmds", content=[f"{xx}"])
    w = t["url"]
    await eod(
        event, f"All 🐼 PetercordPanda 💐 Cmds : [Click Here]({w})", link_preview=False
    )
...
async def heroku_logs(event):
    ...
    await petercordpanda_bot.send_file(
        ...
        caption=f"**💐PetercordPanda 💐 Heroku Logs.**\nPasted [here]({url}) too!",
    )
...
async def def_logs(ult):
    ...
    await petercordpanda_bot.send_file(
        ...
        thumb="resources/extras/petercorpanda.jpg",
        caption=f"**🐼 PetercordPanda 💐 Logs.**\nPasted [here]({url}) too!",
    )
    await xx.edit("Done")
    await xx.delete()
