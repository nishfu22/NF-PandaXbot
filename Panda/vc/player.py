...
\u2022 `.daftarvcg`  show help for commands
🐼  **Admin Commands**:
...
- Repository: [GitHub](https://github.com/ilhammansiz/PandaUserbot)
- Version: PandaUserbot"""
...
async def network_status_changed_handler(gc: GroupCall, is_connected: bool):
    if is_connected:
        ...
        await send_text(f"🐼 Berhasil join voice chat grup")
    else:
        await send_text(f"🐼 berhasil turun voice chat grup")
        mp.chat_id = None
...
@Client.on_message(
    ...
    & filters.regex("^(\\/|.)daftarvcg$")
)
async def show_help(_, m: Message):
    if mp.msg.get("daftarvcg") is not None:
        await mp.msg["daftarvcg"].delete()
    mp.msg["daftarvcg"] = await m.reply_text(USERBOT_HELP, quote=False)
    await m.delete()
...
@Client.on_message(main_filter & self_or_contact_filter & filters.regex("^.joinvc$"))
async def join_group_call(client, m: Message):
    group_call = mp.group_call
    group_call.client = client
    if group_call.is_connected:
        await m.reply_text(f"🐼 Ok saya telah join vc 😎")
        return
...
@Client.on_message(
    main_filter & self_or_contact_filter & current_vc & filters.regex("^.leave$")
)
...
@Client.on_message(main_filter & self_or_contact_filter & filters.regex("^.vc$"))
...
@Client.on_message(
    main_filter & self_or_contact_filter & current_vc & filters.regex("^.stop$")
)
...
@Client.on_message(
    main_filter & self_or_contact_filter & current_vc & filters.regex("^.replay$")
)
...
@Client.on_message(
    main_filter & self_or_contact_filter & current_vc & filters.regex("^.pause")
)
...
@Client.on_message(
    main_filter & self_or_contact_filter & current_vc & filters.regex("^.resume")
)
...
@Client.on_message(
    main_filter & self_or_contact_filter & current_vc & filters.regex("^.clean$")
)
...
@Client.on_message(
    main_filter & self_or_contact_filter & current_vc & filters.regex("^.mute$")
)
...
@Client.on_message(
    main_filter & self_or_contact_filter & current_vc & filters.regex("^.unmute$")
)
