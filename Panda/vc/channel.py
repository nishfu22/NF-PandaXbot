...
main_filter = filters.chat("me") & filters.text & ~filters.edited & ~filters.via_bot
...
async def network_status_changed_handler(gc: GroupCall, is_connected: bool):
    if is_connected:
        ...
        await mp.group_call.client.send_message(
            "me", f"{emoji.CHECK_MARK_BUTTON} joined the voice chat"
        )
    else:
        mp.chat_id = None
        await mp.group_call.client.send_message(
            "me", f"{emoji.CROSS_MARK_BUTTON} left the voice chat"
        )
...
@Client.on_message(main_filter & filters.command("join", prefixes="!"))
async def join_voice_chat(client, m: Message):
    ...
    if 2 <= len_command <= 4:
        ...
        if group_call.is_connected:
            ...
            await group_call.start(channel, join_as=join_as, invite_hash=invite_hash)
...
@Client.on_message(main_filter & filters.regex("^!vc$"))
async def list_voice_chat(client, m: Message):
    ...
    if group_call.is_connected:
        ...
        await m.reply_text(
            f"{emoji.NO_ENTRY} didn't join any voice chat yet", quote=True
        )
...
async def play_track(client, m: Message):
    ...
    if len(playlist) == 1:
        m_status = await m.reply_text(
            f"{emoji.INBOX_TRAY} downloading and transcoding...", quote=True
        )
...
@Client.on_message(main_filter & filters.regex("^(\\/|!)current$"))
...
@Client.on_message(main_filter & filters.regex("^(\\/|!)help$"))
async def show_help(_, m: Message):
    if mp.msg.get("help") is not None:
        await mp.msg["help"].delete()
    mp.msg["help"] = await m.reply_text(USERBOT_HELP, quote=True, parse_mode="md")
    await m.delete()


@Client.on_message(main_filter & filters.command("skip", prefixes="!"))
async def skip_track(_, m: Message):
    ...
    if len(m.command) == 1:
        ...
        try:
            ...
            reply = await m.reply_text(
                f"{emoji.NO_ENTRY} invalid input",
                quote=True,
...
async def stop_playing(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.STOP_BUTTON} stopped playing", quote=True)
...
@Client.on_message(main_filter & filters.regex("^!pause"))
async def pause_playing(_, m: Message):
    mp.group_call.pause_playout()
    await mp.update_start_time(reset=True)
    reply = await m.reply_text(f"{emoji.PLAY_OR_PAUSE_BUTTON} paused", quote=True)
    mp.msg["pause"] = reply
    await m.delete()


@Client.on_message(main_filter & filters.regex("^!resume"))
async def resume_playing(_, m: Message):
    mp.group_call.resume_playout()
    reply = await m.reply_text(f"{emoji.PLAY_OR_PAUSE_BUTTON} resumed", quote=True)
...
@Client.on_message(main_filter & filters.regex("^!clean$"))
async def clean_raw_pcm(client, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.WASTEBASKET} cleaned {count} files", quote=True)
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def unmute(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.SPEAKER_MEDIUM_VOLUME} unmuted", quote=True)
    await _delay_delete_messages((reply, m), DELETE_DELAY)


@Client.on_message(main_filter & filters.regex("^(\\/|!)repo$"))
...
async def download_audio(m: Message):
    ...
    raw_file = os.path.join(
        client.workdir, DEFAULT_DOWNLOAD_DIR, f"{m.audio.file_unique_id}.raw"
    )
