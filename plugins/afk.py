...
@petercordpanda_bot.on(events.NewMessage(outgoing=True))
@petercordpanda_bot.on(events.MessageEdited(outgoing=True))
    ...
    if "afk" not in current_message and "yes" in USER_AFK:
        try:
            if pic.endswith((".tgs", ".webp")):
                shite = await petercordpanda_bot.send_message(event.chat_id, file=pic)
                shites = await petercordpanda_bot.send_message(
                    event.chat_id,
                    get_string("afk_1").format(total_afk_time),
                )
            else:
                shite = await petercordpanda_bot.send_message(
                    event.chat_id,
                    get_string("afk_1").format(total_afk_time),
                    file=pic,
                )
        ...
        try:
            try:
                if pic.endswith((".tgs", ".webp")):
                    await petercordpanda_bot.send_message(LOG, file=pic)
...
async def _(event):
    ...
    if reply:
        ...
        pic = None
    if not USER_AFK:
        last_seen_status = await petercordpanda_bot(
        ...
        if reason:
            try:
                if pic.endswith((".tgs", ".webp")):
                    await petercordpanda_bot.send_message(event.chat_id, file=pic)
                    await petercordpanda_bot.send_message(
                        event.chat_id,
        ...
        try:
            if reason and pic:
                ...
                if pic.endswith((".tgs", ".webp")):
                    await petercordpanda_bot.send_message(LOG, file=pic)
                    await petercordpanda_bot.send_message(LOG, get_string("afk_8"))
                else:
                    await petercordpanda_bot.send_message(
                        LOG, get_string("afk_8"), file=pic
                    )
            else:
                await petercordpanda_bot.send_message(LOG, get_string("afk_8"))
        except BaseException:
