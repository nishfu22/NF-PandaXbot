...
@ilhammansiz_cmd(pattern="addnsfw ?(.*)", admins_only=True)
async def addnsfw(e):
...
@ilhammansiz_cmd(pattern="remnsfw", admins_only=True)
async def remnsfw(e):
    rem_nsfw(e.chat_id)
...
async def checknsfw(e):
    ...
    if action and udB.get("DEEP_API") and e.media:
        pic, name, nsfw = "", "", 0
        try:
            pic = await petercordpanda_bot.download_media(e.media, thumb=-1)
        ...
        if nsfw:
            ...
            if NWARN.get(e.sender_id):
                ...
                if "mute" in action:
                    ...
                    try:
                        await petercordpanda_bot.kick_participant(chat, e.sender_id)
                        await petercordpanda_bot.send_message(
                            chat,
                            f"NSFW Warn 3/3 to [{e.sender.first_name}](tg://user?id={e.sender_id})\n\n**Action Taken** : {action}",
                        )
