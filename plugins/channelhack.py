...
from PandaX_Userbot.functions.ch_db import *
    ...
    for ys in y:
        try:
            if e.text and not e.media:
                await petercordpanda_bot.send_message(int(ys), e.text)
            elif e.media and e.text:
                await petercordpanda_bot.send_file(int(ys), e.media, caption=e.text)
            else:
                await petercordpanda_bot.send_file(int(ys), e.media)
        except Exception as e:
            await petercordpanda_bot.send_message(bot.me.id, str(e))


@ilhammansiz_cmd(pattern="shift (.*)")
async def _(e):
    ...
    try:
        ...
        try:
            c = (await petercordpanda_bot.get_entity(a)).id
        except Exception:
    ...
    try:
        ...
        try:
            d = (await petercordpanda_bot.get_entity(b)).id
        except Exception:
            await z.edit("invalid Channel given")
            return
    async for msg in petercordpanda_bot.iter_messages(int(c), reverse=True):
        try:
            await asyncio.sleep(2)
            await petercordpanda_bot.send_message(int(d), msg)
        except BaseException:
            pass
    await z.edit("Done")


@ilhammansiz_cmd(pattern="asource (.*)")
async def source(e):
    x = e.pattern_match.group(1)
    try:
        y = int(x)
...
@ilhammansiz_cmd(pattern="dsource ?(.*)")
async def dd(event):
...
@ilhammansiz_cmd(pattern="listsource")
...
@ilhammansiz_cmd(pattern="listdest")
async def list_all(event):
    x = await eor(event, "`Calculating...`")
