...
try:
    ...
    telegraph.create_account(short_name="PandaX_Userbot")

_copied_msg = {}


@ilhammansiz_cmd(pattern="kickme$")
async def leave(ult):
...
async def _(event):
    result = await petercordpanda_bot(GetAdminedPublicChannelsRequest())
    output_str = ""
    r = result.chats
...
async def stats(
    ...
    unread = 0
    dialog: Dialog
    async for dialog in petercordpanda_bot.iter_dialogs():
    ...
    try:
        ...
        ct = 0
    try:
        sp = await petercordpanda_bot(GetAllStickersRequest(0))
        sp_count = len(sp.sets)
    except BaseException:
        sp_count = 0
    full_name = inline_mention(await petercordpanda_bot.get_me())
...
async def _(event):
    ...
    try:
        ok = await petercordpanda_bot.inline_query(asst.me.username, q)
...
@ilhammansiz_cmd(pattern="ipinfo ?(.*)")
