...
async def igif(e):
    ...
    z = await petercordpanda_bot.download_media(a.media)
    try:
...
@ilhammansiz_cmd(pattern="invertgif$")
async def igif(e):
    a = await e.get_reply_message()
    if not (a and a.media):
        return await eod(e, "`Reply To gif only`")
...
async def gifs(ult):
    ...
    gifs = await petercordpanda_bot.inline_query("gif", get)
    if not n:
        await gifs[xx].click(
