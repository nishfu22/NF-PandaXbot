...
async def waifu(animu):
    ...
    try:
        sticcers = await petercordpanda_bot.inline_query(
            "stickerizerbot",
...
@ilhammansiz_cmd(pattern="packkang")
async def pack_kangish(_):
    ...
    if len(_.text) > 9:
        ...
        _packname = f"Panda Kang Pack By {_.sender_id}"
    if _e and _e.media and _e.media.document.mime_type == "image/webp":
        ...
        _get_stiks = await petercordpanda_bot(
...
async def hehe(args):
    ...
    user = await petercordpanda_bot.get_me()
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            photo = io.BytesIO()
            photo = await petercordpanda_bot.download_media(message.photo, photo)
            ...
            await xx.edit(f"`{random.choice(KANGING_STR)}`")
            await petercordpanda_bot.download_file(
    ...
    if photo:
        ...
        if (
            ...
            async with petercordpanda_bot.conversation("@Stickers") as conv:
                await conv.send_message("/addsticker")
                await conv.get_response()
                await petercordpanda_bot.send_read_acknowledge(conv.chat_id)
...
async def ultdestroy(event):
    ...
    if not (
        ...
        return await eor(event, "`Reply to Animated Sticker only`")
    await event.client.download_media(ult, "petercordpanda.tgs")
    xx = await eor(event, "`Processing...`")
    os.system("lottie_convert.py petercordpanda.tgs json.json")
    json = open("json.json")
    jsn = json.read()
    json.close()
    jsn = (
    ...
    os.system("lottie_convert.py json.json petercordpanda.tgs")
    await event.client.send_file(
        event.chat_id,
        file="petercordpanda.tgs",
        force_document=False,
        reply_to=event.reply_to_msg_id,
    )
    await xx.delete()
...
async def ultiny(event):
    ...
    if not (reply and (reply.media)):
        ...
        return
    xx = await eor(event, "`processing...`")
    ik = await petercordpanda_bot.download_media(reply)
    im1 = Image.open("resources/extras/petercordpanda_blank.png")
    if ik.endswith(".tgs"):
