...
from PandaX_Userbot.functions.blacklist_db import *

from . import *


@ilhammansiz_cmd(pattern="blacklist ?(.*)", admins_only=True)
async def af(e):
...
@ilhammansiz_cmd(pattern="remblacklist ?(.*)", admins_only=True)
async def rf(e):
    wrd = e.pattern_match.group(1)
    chat = e.chat_id
    if not wrd:
...
@ilhammansiz_cmd(pattern="listblacklist$", admins_only=True)
async def lsnote(e):
