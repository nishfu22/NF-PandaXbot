...
from PandaX_Userbot.functions.forcesub_db import *
...
async def forcesub(ult):
    ...
    try:
        await petercordpanda_bot(GetParticipantRequest(int(joinchat), user.id))
    except UserNotParticipantError:
        await petercordpanda_bot.edit_permissions(ult.chat_id, user.id, send_messages=False)
        res = await petercordpanda_bot.inline_query(
            asst.me.username, f"fsub {user.id}_{joinchat}"
        )
        await res[0].click(ult.chat_id, reply_to=ult.action_message.id)


@ilhammansiz_cmd(pattern="fsub ?(.*)", admins_only=True, groups_only=True)
async def addfor(e):
    ...
    try:
        match = (await petercordpanda_bot.get_entity(ch)).id
    except BaseException:
        return await eod(e, "Give Correct Channel Username or id")
...
@ilhammansiz_cmd(pattern="remfsub$")
async def remor(e):
...
async def fcall(e):
    match = e.pattern_match.group(1)
    spli = match.split("_")
    user = await petercordpanda_bot.get_entity(int(spli[0]))
    cl = await petercordpanda_bot.get_entity(int(spli[1]))
    text = f"Hi [{user.first_name}](tg://user?id={user.id}), You Need to Join"
    text += f" {cl.title} in order to Chat in this Group."
    if not cl.username:
        el = (await petercordpanda_bot(ExportChatInviteRequest(cl))).link
    else:
...
async def diesoon(e):
    ...
    try:
        await petercordpanda_bot(GetParticipantRequest(int(spli[1]), int(spli[0])))
    except UserNotParticipantError:
