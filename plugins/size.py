...
async def size(e):
    ...
    if hasattr(r.media, "document"):
        img = await petercordpanda_bot.download_media(r, thumb=-1)
    else:
        img = await petercordpanda_bot.download_media(r.media)
    im = Image.open(img)
    x, y = im.size
...
@ilhammansiz_cmd(pattern="resize ?(.*)")
async def size(e):
    r = await e.get_reply_message()
    if not (r and r.media):
        return await eor(e, "`Reply To image`")
